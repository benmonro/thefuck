...
from therandy import corrector, const
from therandy.system import Path
from therandy.types import Command
from therandy.corrector import get_corrected_commands, organize_commands
...
class TestGetRules(object):
    ...
    def glob(self, mocker):
        results = {}
        mocker.patch('therandy.system.Path.glob',
                     new_callable=lambda: lambda *_: results.pop('value', []))
        return lambda value: results.update({'value': value})

    @pytest.fixture(autouse=True)
    def load_source(self, monkeypatch):
        monkeypatch.setattr('therandy.types.load_source',
                            lambda x, _: Rule(x))
