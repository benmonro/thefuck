...
from therandy.utils import default_settings, \
...
class TestGetCloseMatches(object):
    @patch('therandy.utils.difflib_get_close_matches')
...
def get_aliases(mocker):
    mocker.patch('therandy.shells.shell.get_aliases',
                 return_value=['vim', 'apt-get', 'fsck', 'randy'])
...
def test_get_all_executables():
    ...
    assert 'randy' not in all_callables
...
def test_get_all_executables_pathsep(path, pathsep):
    with patch('therandy.utils.Path') as Path_mock:
        get_all_executables()
...
class TestCache(object):
    ...
    def shelve(self, mocker):
        ...
        mocker.patch('therandy.utils.shelve.open', new_callable=lambda: _Shelve)
        return value

    @pytest.fixture(autouse=True)
    def enable_cache(self, monkeypatch, shelve):
        monkeypatch.setattr('therandy.utils.cache.disabled', False)
        _cache._init_db()

    @pytest.fixture(autouse=True)
    def mtime(self, mocker):
        mocker.patch('therandy.utils.os.path.getmtime', return_value=0)
    ...
    def key(self, monkeypatch):
        monkeypatch.setattr('therandy.utils.Cache._get_key',
                            lambda *_: 'key')
        return 'key'
...
class TestGetValidHistoryWithoutCurrent(object):
    ...
    def history(self, mocker):
        return mocker.patch('therandy.shells.shell.get_history',
                            return_value=['le cat', 'randy', 'ls cat',
                                          'diff x', 'nocommand x', u'café ô'])

    @pytest.fixture(autouse=True)
    def alias(self, mocker):
        return mocker.patch('therandy.utils.get_alias',
                            return_value='randy')
    ...
    def bins(self, mocker):
        ...
        return mocker.patch('therandy.utils.Path', return_value=path_mock)

    @pytest.mark.parametrize('script, result', [
        ('le cat', ['ls cat', 'diff x', u'café ô']),
        ('diff x', ['ls cat', u'café ô']),
        ('randy', ['ls cat', 'diff x', u'café ô']),
