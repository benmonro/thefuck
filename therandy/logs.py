...
def how_to_configure_alias(configuration_details):
    print(u"Seems like {bold}randy{reset} alias isn't configured!".format(
        bold=color(colorama.Style.BRIGHT),
        reset=color(colorama.Style.RESET_ALL)))
    ...
    if configuration_details:
        ...
        if configuration_details.can_configure_automatically:
            print(
                u"Or run {bold}randy{reset} second time for configuring"
...
def already_configured(configuration_details):
    print(
        u"Seems like {bold}randy{reset} alias already configured!\n"
...
def configured_successfully(configuration_details):
    print(
        u"{bold}randy{reset} alias configured successfully!\n"
        u"For applying changes run {bold}{reload}{reset}"
        u" or restart your shell.".format(
            bold=color(colorama.Style.BRIGHT),
            reset=color(colorama.Style.RESET_ALL),
...
def version(therandy_version, python_version, shell_info):
    sys.stderr.write(
        u'The Randy {} using Python {} and {}\n'.format(therandy_version,
                                                        python_version,
                                                        shell_info))
